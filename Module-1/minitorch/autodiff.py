...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        """
        ls = []
        derv = cls.backward(ctx, d_output)
        if (isinstance(derv, float)):
            derv = [derv]
        for i in range(len(inputs)):
            if (is_constant(inputs[i])):
                pass
            else:
                ls.append(VariableWithDeriv(inputs[i], derv[i]))
        return ls
...
def backpropagate(final_variable_with_deriv):
    ...
    while(len(q) > 0):
        v = q.pop()
        if (is_leaf(v.variable)):
            v.variable._add_deriv(v.deriv)
        else:
            ctx = v.variable.history.ctx
            inputs = v.variable.history.inputs
            d = v.deriv
            var = v.variable.history.last_fn().chain_rule(ctx=ctx, inputs=inputs, d_output=d)
            for i in var:
                q.append(i)
